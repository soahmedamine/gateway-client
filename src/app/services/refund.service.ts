...
import { KeycloakServiceService } from 'src/keycloak-service.service';
...
getRefunds(): Observable<Refund[]> {
  return this.keycloakAuth.getAccessToken().pipe(
    switchMap(token => {
      ...
      return this.http.get<Refund[]>(this.baseUrl, { headers });
    })
  );
}
  addRefund(refund: Refund): Observable<Refund> {
    return this.keycloakAuth.getAccessToken().pipe(
      switchMap(token => {
        ...
        return this.http.post<Refund>(this.baseUrl, refund, { headers });
      })
    );
  }
  ...
  updateRefund(id: number, refund: Refund): Observable<Refund> {
    return this.keycloakAuth.getAccessToken().pipe(
      switchMap(token => {
        ...
        return this.http.put<Refund>(`${this.baseUrl}/${id}`, refund, { headers });
      })
    );
  }
